...
### Phase 1: Delete Debug Files (Low Risk)
```bash
rm debug_hands.py debug_hello1.py debug_hello1_bounds.py debug_normalization.py
rm analyze_hand_proportions.py analyze_pose_structure.py check_arm_lengths.py check_blue_dot.py
```

### Phase 2: Delete Test Files (Low Risk)
```bash
rm test_placeholder_hands.py test_reference_normalization.py test_visual_normalization.py test_frame_range_extraction.py
```

### Phase 3: Delete Migration Scripts (Migration Complete)
```bash
rm -rf scripts/migration_*.py scripts/verify_migration.py scripts/final_validation.py scripts/test_registry_loader.py
```

### Phase 4: Archive Setup Scripts
```bash
mkdir -p archive/setup
mv setup_dataset.py extract_wlasl_videos.py create_test_videos.py optimize_frame_range.py archive/setup/
```

### Phase 5: Convert ARCHITECTURE.py to Markdown
```bash
# Content should become docs/ARCHITECTURE.md
rm ARCHITECTURE.py
```
...
### 2. Add __init__.py to utils/
Currently missing, causing import issues in some contexts.

### 3. Consolidate Test Suite
Create `tests/` folder with:
- `test_normalization.py` (combine test_all_signatures + test_reference_normalization)
- `test_recognition.py` (test_recognition_quality)
- `test_quality_gates.py` (embedding quality)
...
- [ ] Delete 8 debug files (Phase 1)
- [ ] Delete 4 test files (Phase 2)  
- [ ] Delete 5 migration scripts (Phase 3)
- [ ] Archive 4 setup scripts (Phase 4)
- [ ] Convert ARCHITECTURE.py to docs/ARCHITECTURE.md
- [ ] Add utils/__init__.py
- [ ] Create tests/ folder structure
- [ ] Merge recognition engine base class
- [ ] Update quick_start.py or convert to README section
